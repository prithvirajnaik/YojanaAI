...
// server.js (Patched + Improved Version)
const express = require('express');
const bodyParser = require('body-parser');
const fs = require('fs');
const path = require('path');
...
app.post("/recommend", (req, res) => {
  ...
  if (body.text) {
    const fuseRes = fuse.search(body.text);
    console.log('FUSE MATCHES:', fuseRes.length);
    if (fuseRes && fuseRes.length > 0) {
      candidates = fuseRes.slice(0, 200).map(r => r.item);
    } else {
      console.log('Fuse returned 0 results — falling back to full dataset');
      candidates = schemes;
    }
  }
  ...
  const filtered = candidates.filter(s => {
    try {
      return (
        ...
        // DO NOT hard filter target groups anymore!
      );
    } catch (e) {
      return false;
    }
  ...
  console.log("STRICT FILTER MATCHES:", filtered.length);

  // ------------------ Fallback if no strict matches ------------------
  if (filtered.length === 0) {
    console.log("Using fuzzy fallback...");
    const fuzzy = fuse.search(body.text || "").slice(0, 8).map(r => r.item);
    return res.json({
      count: fuzzy.length,
      items: fuzzy,
      fallback: true,
  ...
  const scored = filtered.map(s => {
    ...
    if (checkTargets(user.tags, s.target_groups)) score += 5;
    if (checkState(user.state, s.state_or_scope)) score += 3;
    if (checkGender(user.gender, s.gender)) score += 2;
    if (checkIncome(user.income, parseSchemeIncome(s.income_limit))) score += 2;
    if (s.details && s.details.length < 300) score += 1;
  ...
  const top = scored.slice(0, 8).map(s => s.scheme);
